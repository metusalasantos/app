# -*- coding: utf-8 -*-
"""Análise Olist: Storytelling de Dados
...
### Subtask:
Certifique-se de que todos os arquivos necessários (o script Python principal e os arquivos CSV de dados) estejam organizados em um único diretório. Você precisará exportar esses arquivos do Colab.

#### Instructions
1.  **Baixe o script Python** do seu notebook Colab. Para fazer isso, clique em 'File' (Arquivo) no menu superior, depois em 'Download' (Baixar) e selecione `.py`. Salve o arquivo como `app.py` (ou um nome similar) em um diretório local.
2.  **Baixe os seguintes arquivos CSV** do ambiente do Colab para o mesmo diretório local onde você salvou seu script Python:
    ...
    *   `olist_products_dataset.csv`
    ...
    Você pode baixar cada arquivo individualmente clicando no ícone de pasta (navegador de arquivos) à esquerda, navegando até o arquivo e clicando nos três pontos ao lado dele para selecionar 'Download'.
...
### Subtask:
Crie um arquivo chamado `requirements.txt` na raiz do seu projeto que liste todas as bibliotecas Python utilizadas pela sua aplicação.
...
"""#### Instructions
1. O arquivo `requirements.txt` foi gerado. Agora, baixe o arquivo `requirements.txt` recém-criado para o mesmo diretório local onde você salvou seu script Python e os arquivos CSV.

Você pode baixar o arquivo clicando no ícone de pasta (navegador de arquivos) à esquerda, navegando até o arquivo `requirements.txt` e clicando nos três pontos ao lado dele para selecionar 'Download'.

## Escolher uma Plataforma de Deploy

### Subtask:
Decida onde você quer hospedar sua aplicação Streamlit.

#### Instructions
1. Considere as seguintes plataformas para o deploy da sua aplicação Streamlit:
    *   **Streamlit Community Cloud:** É a opção mais recomendada e fácil para aplicações Streamlit, pois é otimizado para isso. Geralmente requer apenas uma conexão com um repositório GitHub.
    *   **Heroku:** Uma plataforma de nuvem que suporta várias linguagens de programação, incluindo Python. Requer um `Procfile` para definir como sua aplicação deve ser executada.
    *   **Servidores em Nuvem (AWS, Google Cloud, Azure):** Oferecem maior controle e flexibilidade, mas exigem mais conhecimento técnico para configuração (ex: Docker, máquinas virtuais).
2. Para este projeto, é fortemente sugerido o uso do **Streamlit Community Cloud** devido à sua simplicidade e integração direta com o GitHub para aplicações Streamlit.
3. Uma vez que você tenha escolhido a plataforma, você estará pronto para configurar um repositório Git.
...
2. Crie um **novo repositório público** no GitHub. Dê a ele um nome descritivo (ex: `olist-streamlit-app`). Não adicione um README ou .gitignore ainda.
3. Em seu ambiente local (onde você baixou os arquivos): inicie um repositório Git local, adicione todos os arquivos (o script `app.py`, todos os arquivos `.csv`, e o `requirements.txt`), faça um commit e envie-os para o seu novo repositório GitHub.
    *   Abra um terminal na pasta onde os arquivos estão.
    *   Execute `git init`
    *   Execute `git add .`
    *   Execute `git commit -m "Initial commit of Olist Streamlit App"`
    *   Adicione o repositório remoto: `git remote add origin <URL_do_seu_repositorio_GitHub>` (substitua `<URL_do_seu_repositorio_GitHub>` pelo URL HTTPS do seu repositório GitHub).
    *   Envie para o GitHub: `git push -u origin main` (ou `master`, dependendo do branch padrão do seu repositório).

## Realizar o Deploy

### Subtask:
Realize o deploy da sua aplicação Streamlit no Streamlit Community Cloud usando o repositório GitHub configurado.
...
#### Instructions
1. Acesse o Streamlit Community Cloud (share.streamlit.io) e faça login com sua conta GitHub.
2. No seu painel (dashboard), clique em 'New app' (Novo aplicativo).
3. Conecte sua conta GitHub (se ainda não estiver conectada).
4. Selecione o repositório que você criou para sua aplicação (ex: `olist-streamlit-app`).
5. Verifique se o 'Main branch' (Branch principal) está configurado corretamente (geralmente `main` ou `master`).
...
8. Aguarde o Streamlit construir e implantar sua aplicação. Isso pode levar alguns minutos.
...
7. Clique em 'Deploy!' (Implantar!).
8. Aguarde o Streamlit construir e implantar sua aplicação. Isso pode levar alguns minutos.
9. **Após a implantação**, verifique se a aplicação está funcionando corretamente. Copie e compartilhe o URL fornecido pelo Streamlit Community Cloud.
...
### Subtask:
Fornecer as informações sobre o deploy bem-sucedido ou quaisquer problemas encontrados durante o processo, bem como o link da aplicação hospedada, se aplicável.
